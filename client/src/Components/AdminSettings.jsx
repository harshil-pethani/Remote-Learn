...
import { getAllAdminApi, updateAdminDataApi, updateAdminStatus, updatepasswordApi } from '../Config/Api'
...
const AdminSettings = ({ adminDetails, setAdminDetails, setAdminLogged }) => {
    ...
    const acceptAdmin = async (adminId) => {
        if (window.confirm("Are you sure to Accept this Admin ? ")) {
            try {
                setIsLoading(true);
                const res = await axios.put(updateAdminStatus(adminId), {
                    status: "approved"
    ...
    const blockAdmin = async (adminId) => {
        if (window.confirm("Are you sure to Block this Admin ? ")) {
            try {
                setIsLoading(true);
                const res = await axios.put(updateAdminStatus(adminId), {
                    status: "blocked"
                                            ...
                                            <tbody>
                                                {
                                                    adminsData?.map((adminData, index) => (
                                                        <tr key={index}>
                                                            ...
                                                            <td className={"status " + adminData.verified}>
                                                                {adminData.verified.toUpperCase()}
                                                            </td>
                                                            <td>
                                                                <button onClick={() => acceptAdmin(adminData._id)} disabled={adminData.verified === "approved"}>
                                                                    Approve
                                                                </button>
                                                            </td>
                                                            <td>
                                                                <button onClick={() => blockAdmin(adminData._id)} disabled={adminData.verified === "blocked"}>
                                                                    Block
                                                                </button>
                                                            </td>
                                                        </tr>
